...
// const formAction = 'http://localhost:8000/form';
const mapUrl =
...
const ValidationError = ({field, errors}: {field: string, errors: {[key: string]: any}}) => (
  <>
  {errors && errors[field] && <div className='validation-error'>{errors[field]}</div>}
  </>
);
...
class ContactUs extends React.Component<{}, State> {
  state = {
    message: '',
    name: '',
    _replyto: '',
  ...
  validate(field: string | undefined = undefined) {
    ...
    if (field != null) {
      (errors as any)[field] = this.validateField(
        field,
        (this.state as any)[field]
      );
    } else {
      for (let key of fields) {
        (errors as any)[key] = this.validateField(
          key,
